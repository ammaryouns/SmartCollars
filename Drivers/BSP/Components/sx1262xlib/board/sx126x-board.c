...
{
    SX126x.Spi.handle  =   &SX126x_SPI_Handle;
    SX126x.Spi.Nss     =  (Gpio_t){.port =  SX1262_NSS_GPIO_Port, .pin  = SX1262_NSS_Pin };
    SX126x.Reset       =  (Gpio_t){.port =  SX1262_RESET_GPIO_Port, .pin  = SX1262_RESET_Pin };
    SX126x.BUSY        =  (Gpio_t){.port =  SX1262_BUSY_GPIO_Port, .pin  = SX1262_BUSY_Pin };
    SX126x.DIO1        =  (Gpio_t){.port =  SX1262_DIO1_GPIO_Port, .pin  = SX1262_DIO1_Pin,  .IRQn = SX1262_DIO1_EXTI_IRQn};
    SX126x.DIO2        =  (Gpio_t){.port =  NULL,  .pin  = NULL };
    SX126x.DIO3        =  (Gpio_t){.port =  NULL,  .pin  = NULL }; 

    AntPow = (Gpio_t){.port =  SX1262_ANT_SW_GPIO_Port, .pin  = SX1262_ANT_SW_Pin };
...
void SX126x_SPI_Init( void )
{
  ...
  if(HAL_SPI_GetState( SX126x.Spi.handle) == HAL_SPI_STATE_RESET )
  {
    HAL_SPI_Init(SX126x.Spi.handle);
  }  
}
/**
 * @brief  Configures sensor SPI interface.
 ...
 * @retval 
 */
void SX126x_SPI_DeInit( void )
{
 
  if(HAL_SPI_GetState( SX126x.Spi.handle) != HAL_SPI_STATE_RESET )
  {
    HAL_SPI_DeInit(SX126x.Spi.handle);
  }  
}
void SX126xIoDeInit( void )
{
  GPIO_PinDeInit(SX126x.Spi.Nss);
  GPIO_PinDeInit(SX126x.BUSY);
  GPIO_PinDeInit(SX126x.DIO1);
...
void SX126xWakeup( void )
{
    ...
    uint8_t status = SpiInOut( &SX126x.Spi, 0x00 );
