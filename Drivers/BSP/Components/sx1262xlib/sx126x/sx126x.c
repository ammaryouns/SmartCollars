...
void SX126xSetTxParams( int8_t power, RadioRampTimes_t rampTime )
{
    ...
    if( SX126xGetPaSelect( 0 ) == SX1261 )
    {
        ...
        OCP_config = 0x18;
        SX126xWriteRegister( REG_OCP, OCP_config ); // current max is 80 mA for the whole device
    }
    else // sx1262
    {
        ...
        OCP_config = 0x38;
        SX126xWriteRegister( REG_OCP, OCP_config ); // current max 160mA for the whole device
        
    }
    
#ifdef RADIO_UNIT_TEST
    int test = SX126xReadRegister( REG_OCP );
    if (test == OCP_config)
    {
      printTask("Radio chip communication with MCU (SPI ok)");
    }
    else
    {
      printTask("Radio chip communication with MCU (SPI Not ok)");
    }
...
//if(modulationParams->Params.LoRa.LowDatarateOptimize != 0)
//{
//  printDEBUG("modulationParams->Params.LoRa.LowDatarateOptimize %d \r\n", modulationParams->Params.LoRa.LowDatarateOptimize);
//}
        SX126xWriteCommand( RADIO_SET_MODULATIONPARAMS, buf, n );
